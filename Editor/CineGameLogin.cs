...
namespace CineGame.Host.Editor
{
    public class CineGameLogin : EditorWindow
    {
        ...
        private static void OnLoad () {
            ...
            var _marketSlugs = CineGameSDK.MarketSlugMap.Values.ToList ();
        ...
        public static bool GetAccessToken (string userName, string userPassword) {
            ...
            var env = "production";
            if (CurrentMarketSlug.StartsWith ("DEV_")) {
                env = "dev";
            } else if (CurrentMarketSlug.StartsWith ("STAGING_")) {
                env = "staging";
            }
            Configuration.NODE_ENV = env;
                ...
                try {
                    ...
                    */
                    ...
                    foreach (var id in MarketIdsAvailable) {
                        appNames.Add (CineGameSDK.MarketSlugMap [id]);
                    }
